...
contract('DappToken', function(accounts) {
  ...
  it('initializes the contract with the correct config values', function() {
    return DappToken.deployed().then(function(instance) {
      ...
      assert.equal(name, 'Research Token', 'has the correct name');
      return tokenInstance.symbol()
    }).then(function(symbol) {
      assert.equal(symbol, 'RCH', 'has the correct symbol');
      return tokenInstance.standard();
    }).then(function(standard) {
      assert.equal(standard, 'RCH Token v1.0', 'has the correct standard');
